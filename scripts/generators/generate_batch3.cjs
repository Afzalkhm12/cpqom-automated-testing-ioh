...
function toPrefix(title) {
  ...
  const tail = words[words.length - 1]
    ...
    .substring(0, 3)
    .toUpperCase();
  const raw = (initials + tail).replace(/(.)\1+/g, "$1");
...
function toSlugShort(title) {
  return toSlug(title).substring(0, 40);
}
...
function generateSpec(spec) {
  ...
  if (payload) {
    ...
    const msisdnFields = detectMsisdnFields(payload);
...
function buildBody(msisdn) {
  ...
  if (payload.Input && payload.Input.msisdn) payload.Input.msisdn = msisdn;
  if (payload.Request && payload.Request.MSISDN) payload.Request.MSISDN = msisdn;
  if (payload.MSISDNPerNIKReq && payload.MSISDNPerNIKReq.MSISDN) payload.MSISDNPerNIKReq.MSISDN = msisdn;
