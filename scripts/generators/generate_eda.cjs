...
const specs = JSON.parse(
  fs.readFileSync("scripts/generators/eda_specs.json", "utf8")
);
...
function toPrefix(title) {
  // EDA titles like "Activate Profile PCRF RFS_NEWREG_PCRF ADD"
  // Extract key words: action + target + operation
  const words = title
    .trim()
    .split(/[\s_]+/)
    .filter(Boolean);
  // Take first 3 meaningful words' initials + last word initial
  const meaningful = words.filter(
    (w) =>
      !["HLR", "PCRF", "RFS", "ADD", "DEL", "SET"].includes(w.toUpperCase())
  );
  const initials = meaningful
    .slice(0, 4)
    .map((w) => w[0].toUpperCase())
    .join("");
  const op = words[words.length - 1].toUpperCase(); // ADD/DEL/SET
  const opChar =
    op === "ADD" ? "A" : op === "DEL" ? "D" : op === "SET" ? "S" : "";
  return (initials + opChar).substring(0, 10);
}
...
// For EDA, MSISDN is nested in MOId array - need special injection
function buildMsisdnInjection() {
  return `  // Inject MSISDN into MOId array
...
function generateSpec(spec) {
  ...
  const envKey = toEnvKey(title).substring(0, 45);
  const dbKey = slug.substring(0, 40);
  const epic = `API Readiness EDA`;
...
test("TC-${prefix}-004 — Negative Scenario", async ({ request }) => {
  ...
  let status;
  await test.step("TC-${prefix}-004_S01 - ${METHOD} data invalid/MSISDN kosong", async () => {
...
for (const spec of specs) {
  ...
  const filePath = `tests/api-readiness/eda/${slug}.spec.js`;
