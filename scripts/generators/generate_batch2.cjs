...
function toPrefix(title) {
  const words = title
    ...
    .split(/[\s\/\(\)]+/)
    .filter(Boolean);
  // Take first letters of first 4 words, plus 2-3 chars of last word
  ...
  const tail = words[words.length - 1]
    ...
    .substring(0, 3)
    .toUpperCase();
  const raw = (initials + tail).replace(/(.)\1+/g, "$1"); // deduplicate consecutive same chars
...
function detectMsisdnFields(payload, prefix = "") {
  const fields = [];
  function scan(obj, path) {
    if (!obj || typeof obj !== "object") return;
    for (const [k, v] of Object.entries(obj)) {
      const fullPath = path ? `${path}.${k}` : k;
      if (typeof v === "object" && v !== null) scan(v, fullPath);
      else if (
...
function generateSpec(spec) {
  ...
  const envKey = toEnvKey(title).substring(0, 40);
  const dbKey = toSlug(title).substring(0, 40);
  const epic = `API Readiness ${group.toUpperCase()}`;
  const msisdnFields = payload
    ? detectMsisdnFields(payload)
    : ["msisdn", "MSISDN"];

  let payloadCode;
  if (payload) {
    let jsonStr = JSON.stringify(payload, null, 4);
    // Inject msisdn dynamically for detected fields
...
for (const spec of specs) {
  ...
  const dir =
    spec.group === "ctlst"
      ? "tests/api-readiness/ctlst"
      : `tests/api-readiness/${spec.group}`;
  const filePath = `${dir}/${slug}.spec.js`;
