...
test("TC004_Create CCA", async () => {
  ...
  await test.step("TC004_S04 - Fill in all mandatory fields", async () => {
    ...
    await page
      ...
      .fill(tc001.accountName + " " + counter);

    await page.getByRole("textbox", { name: "Description" }).click();
    await page
      .getByRole("textbox", { name: "Description" })
      .fill("Created by Automation Testing");
    ...
    await page
      ...
      .fill(tc001.phone.toString());

    await page.getByRole("textbox", { name: "Email" }).click();
    await page
      .getByRole("textbox", { name: "Email" })
      .fill("account.cca." + counter + "@company.co.id");
...
test("Create Billing Contact", async () => {
  ...
  await page.goto(
    `${dataAuth.marketing.afterLoginUrl}lightning/o/Contact/list?filterName=__Recent`
  );
  ...
  await page
    ...
    .fill(`${tcContact.firstName}`);
  await page.getByRole("textbox", { name: "Last Name" }).click();
  await page
    .getByRole("textbox", { name: "Last Name" })
    .fill(`${tcContact.lastName} ${counter}`);
  ...
  await page
    ...
    .fill(tc001.accountName + " " + counter);
  await page
    .getByRole("listbox", { name: "Account Name" })
  ...
  await page
    .getByRole("textbox", { name: "Email" })
    .fill("johan.armando@example.com");
  ...
  await expect(
    page
      .locator("div")
      .filter({ hasText: "Success notification.Contact" })
      .nth(3)
  ).toBeVisible();

  const billingContactId = page
    .url()
...
test("TC005_Create CA under CCA", async () => {
  ...
  await test.step("TC005_S04 - Fill in all mandatory fields", async () => {
    ...
    await page
      ...
      .fill(tc002.idReference + counter.toString().padStart(4, "0"));
    ...
    await page
      .getByRole("combobox", { name: "Line Of Business", exact: true })
      .click();
    await page
      .getByRole("option", { name: "Communications & Technologies" })
      .click();
    ...
    await page
      .getByRole("textbox", { name: "Phone" })
      .fill(tc002.phone.toString());

    await page.getByRole("textbox", { name: "Email" }).click();
    await page
      .getByRole("textbox", { name: "Email" })
      .fill("account.ca." + counter + "@company.co.id");
