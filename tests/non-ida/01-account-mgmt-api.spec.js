...
async function getRecordTypeId(request, sobjectType, developerName) {
    ...
    if (!id) throw new Error(`RecordType '${developerName}' not found for ${sobjectType}`);
    return id;
}

test('API Connection Test', async ({ request }) => {
    const loginResponse = await request.post(`${dataAuth.sysadmin.url}/services/oauth2/token`, {
...
test('Create Billing Contact', async ({ request }) => {
    ...
    await test.step('Look up Billing Contact RecordType Id', async () => {
        recordTypeId = await getRecordTypeId(request, 'Contact', 'SFA_Billing_Contact');
    ...
    await test.step('Create Billing Contact via API', async () => {
        ...
        expect(body.success, 'Billing Contact creation should succeed').toBeTruthy();
        ...
        const body = await sfRequest(request, 'post',
            `${instanceUrl}/services/data/v65.0/sobjects/Account`,
            {
                ...
                data: {
                    ...
                    SFA_License_Number__c: tc002.idReference + counter.toString().padStart(4, '0'),
