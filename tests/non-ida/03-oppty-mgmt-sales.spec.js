...
async function patchContractedMonths(request, instanceUrl, accessToken) {
    ...
    expect(patchResponse.status(), 'Patch NumberOfContractedMonths should return 204 No Content').toBe(204);
...
async function deleteOpportunityLineItems(request, instanceUrl, accessToken) {
    ...
    expect(queryResponse.ok(), 'Query for OpportunityLineItems should succeed').toBeTruthy();
    ...
    for (const item of lineItems) {
        ...
        expect(deleteResponse.status(), `Delete OpportunityLineItem ${item.Id} should return 204 No Content`).toBe(204);
...
test('API Connection Test', async ({ request }) => {
  ...
  // Step 1: Authenticate and get access token
    ...
    if (!opportunityId) {
        ...
        expect(opportunityResponse.ok(), 'Query for latest Scoping opportunity should succeed').toBeTruthy();
...
test('TC010_Managing my opportunity', async () => {
    ...
    await expect(
        page.locator('lightning-formatted-text').filter({ hasText: `${tc010.expectedOtc}` }),
        `OTC value should display '${tc010.expectedOtc}'`
    ).toBeVisible();
    await expect(
        page.locator('lightning-formatted-text').filter({ hasText: `${tc010.expectedMrc}` }),
        `MRC value should display '${tc010.expectedMrc}'`
    ).toBeVisible();
    await expect(
        page.locator('lightning-formatted-text').filter({ hasText: `${tc010.expectedTotal}` }),
        `Total value should display '${tc010.expectedTotal}'`
    ).toBeVisible();
    await expect(
        page.locator('lightning-formatted-text').filter({ hasText: `${tc010.expectedAnnualRevenue}` }),
