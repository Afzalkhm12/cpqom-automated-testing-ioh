/**
 * API Readiness Test — QueryPromotionalPlan
 *
 * Source  : Confluence Page ID 1095499777
...
const BASE_URL =
  process.env.QUERYPROMOTIONALPLAN_URL ??
  "http://bi-air-query-sit.apps.ocpmwdev.ioh.co.id:80/generalgetapi";
const AUTH =
  process.env.QUERYPROMOTIONALPLAN_AUTH ?? "Basic Y3BxdXNlcjpjcHFwYXNzMjAyNQ==";
const BASE_HEADERS = {
...
test.beforeAll(async () => {
  tc = await getTestParams(
    "api_readiness_querypromotionalplan",
    "tc_que",
    userId
  );
...
test("TC-QUE-001 — Endpoint reachable", async ({ request }) => {
  ...
  await allure.feature("QueryPromotionalPlan");
