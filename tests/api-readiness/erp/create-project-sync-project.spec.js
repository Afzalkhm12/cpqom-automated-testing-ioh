/**
 * API Readiness Test — Create Project/Sync Project
 *
 * Source  : Confluence Page ID 984612865
 * Method  : POST | Type: Synchronous
 * SIT URL : http://dev-cgw.ioh.co.id/sit/cpq/erp/createproject
 */
...
const BASE_URL =
  process.env.CREATE_PROJECT_SYNC_PROJECT_URL ??
  "http://dev-cgw.ioh.co.id/sit/cpq/erp/createproject";
const AUTH =
  process.env.CREATE_PROJECT_SYNC_PROJECT_AUTH ??
...
test.beforeAll(async () => {
  tc = await getTestParams(
    "api_readiness_create-project-sync-project",
    "tc_cpsp",
    userId
  );
...
test("TC-CPSP-001 — Endpoint reachable", async ({ request }) => {
  ...
  await allure.feature("Create Project/Sync Project");
