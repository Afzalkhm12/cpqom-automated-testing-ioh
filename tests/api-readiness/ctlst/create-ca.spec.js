/**
 * API Readiness Test — Create CA
 *
 * Source  : Confluence Page ID 635011073
 * Method  : POST | Type: Synchronous
 * SIT URL : http://dev-cgw.ioh.co.id/sit/cpq/createCA
 */
...
const BASE_URL =
  process.env.CREATE_CA_URL ?? "http://dev-cgw.ioh.co.id/sit/cpq/createCA";
const AUTH = process.env.CREATE_CA_AUTH ?? "Basic Y3BxdXNlcjpjcHFwYXNzMjAyNQ==";
const BASE_HEADERS = {
...
test.beforeAll(async () => {
  tc = await getTestParams("api_readiness_create-ca", "tc_ca", userId);
...
test("TC-CA-001 — Endpoint reachable", async ({ request }) => {
  ...
  await allure.feature("Create CA");
  ...
  await test.step("TC-CA-001_S02 - Verifikasi status bukan 404/596", async () => {
    if ([404, 596].includes(status) || BASE_URL === "TBD") {
      ...
      console.warn(
        `[TC-CA-001] ⚠️  Endpoint tidak tersedia (HTTP ${status}) / TBD.`
      );
      test.skip();
      return;
    }
...
test("TC-CA-002 — Schema Check", async ({ request }) => {
  ...
  await test.step("TC-CA-002_S02 - Cek response tidak null", async () => {
...
test("TC-CA-003 — Positive Scenario", async ({ request }) => {
  ...
  await allure.severity("critical");
  await test.step("TC-CA-003_S01 - POST dengan data valid", async () => {
    const r = await postApi(request, buildBody(VALID_MSISDN));
...
test("TC-CA-004 — Negative Scenario", async ({ request }) => {
  ...
  let status;
  await test.step("TC-CA-004_S01 - POST data invalid/kosong", async () => {
    const r = await postApi(request, buildBody(""));
    status = r.response.status();
  });
  await test.step("TC-CA-004_S02 - Verifikasi API memberikan respon", async () => {
...
test("TC-CA-005 — Auth Check", async ({ request }) => {
  ...
  await allure.severity("critical");
  let status;
  await test.step("TC-CA-005_S01 - POST tanpa Authorization header", async () => {
    const r = await postApi(request, buildBody(VALID_MSISDN), {
      headers: { "Content-Type": "application/json" }
    });
    status = r.response.status();
  });
  await test.step("TC-CA-005_S02 - Verifikasi respon 401/403 atau response lain", async () => {
...
test("TC-CA-006 — Performance < 10 detik", async ({ request }) => {
  ...
  await allure.severity("normal");
  let elapsed;
  await test.step("TC-CA-006_S01 - Ukur response time", async () => {
    const t = Date.now();
    await postApi(request, buildBody(VALID_MSISDN));
    elapsed = Date.now() - t;
  });
  await test.step("TC-CA-006_S02 - Verifikasi < 10s", async () => {
