/**
 * API Readiness Test — Query Order Status
 *
 * Source  : Confluence Page ID 5679305
 * Method  : POST | Type: Synchronous
 * SIT URL : http://tm-route-tibco-mashery-dev.apps.ocpmwdev.ioh.co.id/sit/queryorderstatus
 */
...
const BASE_URL =
  process.env.QUERY_ORDER_STATUS_URL ??
  "http://tm-route-tibco-mashery-dev.apps.ocpmwdev.ioh.co.id/sit/queryorderstatus";
const AUTH =
  process.env.QUERY_ORDER_STATUS_AUTH ?? "Basic Y3BxdXNlcjpjcHFwYXNzMjAyNQ==";
const BASE_HEADERS = {
...
test.beforeAll(async () => {
  tc = await getTestParams(
    "api_readiness_query-order-status",
    "tc_queryord",
    userId
  );
...
function buildBody(msisdn) {
  const payload = {
    transid: "6285782735882201710102210",
    siebelDataLocation: "ICARE",
    servicedata: {
      orderType: "Change SIM Card Attributes",
      customerType: "Prepaid",
      epsOrderId: "EPS3251",
      channel: "NEWPOS",
      lang: "EN",
      sourceTransactionId: "23454565765"
    },
    servicename: "QUERY ORDER HISTORY",
    msisdn: "6281510989042"
  };
...
test("TC-QUERYORD-001 — Endpoint reachable", async ({ request }) => {
  ...
  await allure.feature("Query Order Status");
