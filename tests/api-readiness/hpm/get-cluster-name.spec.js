/**
 * API Readiness Test — Get Cluster Name
 *
 * Source  : Confluence Page ID 694943794
 * Method  : POST | Type: Synchronous
 * SIT URL : http://bi-hpm-tibco-others-sit.apps.ocpmwdev.ioh.co.id:80/getclustername
 */
...
const BASE_URL =
  process.env.GET_CLUSTER_NAME_URL ??
  "http://bi-hpm-tibco-others-sit.apps.ocpmwdev.ioh.co.id:80/getclustername";
const AUTH =
  process.env.GET_CLUSTER_NAME_AUTH ?? "Basic Y3BxdXNlcjpjcHFwYXNzMjAyNQ==";
const BASE_HEADERS = {
...
test.beforeAll(async () => {
  tc = await getTestParams("api_readiness_get-cluster-name", "tc_gcn", userId);
...
test("TC-GCN-001 — Endpoint reachable", async ({ request }) => {
  ...
  await allure.feature("Get Cluster Name");
