/**
 * API Readiness Test — Update CCA
 * Source: Confluence Page ID 735117313
 * Method: POST | SIT URL: http://dev-cgw.ioh.co.id/sit/cpq/updateCCA
 */
...
const BASE_URL =
  process.env.UPDATE_CCA_URL ?? "http://dev-cgw.ioh.co.id/sit/cpq/updateCCA";
const AUTH =
  process.env.UPDATE_CCA_AUTH ?? "Basic Y3BxdXNlcjpjcHFwYXNzMjAyNQ==";
const BASE_HEADERS = {
...
test.beforeAll(async () => {
  tc = await getTestParams("api_readiness_update-cca", "tc_uc", userId);
...
test("TC-UC-001 — Endpoint reachable", async ({ request }) => {
  ...
  await allure.feature("Update CCA");
