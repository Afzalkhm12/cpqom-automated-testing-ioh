...
class JiraReporter {
  ...
  async _createJiraTicket(test, result) {
    ...
    const suiteName    = titlePath.length > 1 ? titlePath.slice(0, -1).join(' > ') : testFile;
...
function _resolveLoginUser(filePath) {
  if (process.env.TEST_USER_ADMIN === 'true') return 'sysadmin (TEST_USER_ADMIN=true)';
  const base = path.basename(filePath);
  if (base.includes('account-mgmt') || base.includes('app-navigation')) return 'marketing';
  if (base.includes('lead-mgmt') || base.includes('oppty-mgmt-sales')) return 'salesOperation';
  if (base.includes('oppty-mgmt-es') || base.includes('quote-mgmt-es')) return 'enterpriseSolution';
...
function _flattenSteps(steps, depth = 0, ancestors = []) {
  ...
  for (const step of steps) {
    ...
    if (step.steps?.length) {
      result.push(..._flattenSteps(step.steps, depth + 1, [...ancestors, step.title]));
    }
  }
  return result;
}
...
function _parseAssertionMessage(errorMessage) {
  ...
  const stopKeywords = ['locator:', 'expected:', 'received:', 'call log:', 'expect(', 'at '];
...
function _adfStepList(steps) {
  ...
  if (!topLevel.length) {
    return { type: 'paragraph', content: [{ type: 'text', text: 'No named steps recorded.' }] };
  }
  ...
  return {
    ...
    content: topLevel.map((step) => {
      ...
      const dur    = step.duration != null ? ` [${(step.duration / 1000).toFixed(2)}s]` : '';
      return {
        type: 'listItem',
        content: [{
          type: 'paragraph',
